...
import {
  ...
  CheckCircle2, Plus, ShoppingBag, Lock,
} from 'lucide-react';
...
import {
  ...
  initiateMobileBankingPaymentRequest,
...
const CheckoutPage = () => {
  ...
  const [paymentMethod, setPaymentMethod] = useState('cod'); // 'cod' | 'esewa' | 'mobile_banking'
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      ...
      const data = await placeOrderRequest(orderPayload);
      const orderId = data.order?._id;

      if (paymentMethod === 'esewa') {
        const payment = await initiateEsewaPaymentRequest(orderId);
        window.location.href = payment.paymentUrl;
        return;
      }
      if (paymentMethod === 'mobile_banking') {
        const payment = await initiateMobileBankingPaymentRequest(orderId);
        window.location.href = payment.paymentUrl;
        return;
      }
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 border-b border-stone-200 pb-6 mb-8">
        ...
        <div className="flex items-center gap-3 text-sm">
          <span className="flex items-center gap-1.5 text-primary-600 font-medium">
            <CheckCircle2 className="w-4 h-4" strokeWidth={2} /> Cart
          </span>
          <span className="w-8 h-px bg-stone-300" />
          <span className="flex items-center gap-1.5 text-primary-600 font-medium">
            <CheckCircle2 className="w-4 h-4" strokeWidth={2} /> Review
          </span>
          <span className="w-8 h-px bg-stone-300" />
          <span className="flex items-center gap-1.5 text-ink font-semibold">
            <span className="w-5 h-5 rounded-full bg-primary-600 text-white text-xs flex items-center justify-center">3</span> Checkout
          </span>
        </div>
      </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-3 mb-3">
              <button
                ...
                className={`flex items-center gap-3 border rounded-xl px-4 py-3.5 text-left transition ${
                  paymentMethod === 'cod' ? 'border-primary-500 bg-primary-50/60' : 'border-stone-200 hover:border-stone-300'
                }`}
              >
                <span className={`w-4 h-4 rounded-full border-2 flex-shrink-0 ${paymentMethod === 'cod' ? 'border-primary-600 bg-primary-600' : 'border-stone-300'}`} />
                <Banknote className="w-5 h-5 text-ink/60" strokeWidth={1.75} />
                <span className="font-medium text-ink text-sm">Cash on delivery</span>
              </button>
              <button
                onClick={() => setPaymentMethod((m) => (m === 'cod' ? 'esewa' : m))}
                className={`flex items-center gap-3 border rounded-xl px-4 py-3.5 text-left transition ${
                  paymentMethod !== 'cod' ? 'border-primary-500 bg-primary-50/60' : 'border-stone-200 hover:border-stone-300'
                }`}
              >
